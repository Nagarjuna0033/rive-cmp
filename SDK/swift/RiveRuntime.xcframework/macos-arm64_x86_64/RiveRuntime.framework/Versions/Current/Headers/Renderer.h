...
/// Configuration for a single item within a batched render pass.
///
/// Extends RendererConfiguration with a viewport rectangle that defines where
/// within the shared surface this item should be drawn.
typedef struct
{
    /// The base rendering configuration (artboard, fit, alignment, etc.)
    RendererConfiguration configuration;

    /// The viewport origin X in pixels within the shared surface.
    CGFloat viewportX;

    /// The viewport origin Y in pixels within the shared surface.
    CGFloat viewportY;

    /// The viewport width in pixels within the shared surface.
    CGFloat viewportWidth;

    /// The viewport height in pixels within the shared surface.
...
/**
 * Draws multiple artboard configurations to a single shared Metal texture.
 *
 * Each configuration includes a viewport rectangle defining where within the
 * surface that item should be rendered. All items are drawn in a single
 * beginFrame/flush pass for optimal batching performance.
 *
 * @param configurations A C array of BatchRendererConfiguration structs
 * @param count The number of configurations in the array
 * @param surfaceWidth The width of the shared surface in pixels
 * @param surfaceHeight The height of the shared surface in pixels
 * @param surfaceClearColor The clear color for the surface in ARGB format
 * @param texture The Metal texture to render to
 * @param device The Metal device
 * @param finalize A block called after drawing completes with the command buffer
 * @param onError A block called if drawing fails
 */
